...
const UserProfilePage = () => {
  ...
  const handleUpdatePassword = (e) => {
    ...
    changePassword(dispatch, user.userId, oldPassword, newPassword, token).then(
      (data) => {
        console.log("ok1");
  ...
  const handleDeleteAvatar = (e) => {
    ...
    deleteImage(loginReducer.user.avatar, token).then((result) => {
      if (result) {
        const newUser = {
          userId: user.userId,
  ...
  const urlAvatarCurrent =
    "http://127.0.0.1:8081/api/file/avatar/" + loginReducer.user.avatar;
  // de show cac button
